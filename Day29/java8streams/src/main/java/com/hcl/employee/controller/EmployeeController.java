...
public class EmployeeController {
   ...
   public ModelAndView searchEmployee(ModelAndView model,@ModelAttribute Employee searchemployee) {
           ...
           listemployee = listemployee.stream().filter((var)-> (var.getEmpname().startsWith(searchemployee.getEmpname()))
        		   && (searchemployee.getBand() == '0' || var.getBand()==searchemployee.getBand() )
        		   && (searchemployee.getSalary() == 0.0 || var.getSalary()>searchemployee.getSalary())).collect(Collectors.toList());
